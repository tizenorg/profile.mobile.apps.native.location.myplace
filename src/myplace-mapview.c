...
static void showPositionOnMapsService(myplace_app_data *ad)
{
	...
	map_view_set_zoom_level(ad->maps_view, 12);

	if (ad->mapview_place->address != NULL)
		elm_entry_entry_set(ad->map_entry, ad->mapview_place->address);

	ret = map_view_set_center(ad->maps_view, maps_coord);
	if (ret != MAPS_ERROR_NONE) {
		LS_LOGE("map_view_set_center fail, error = %d", ret);
		return;
	}
	ret = map_view_remove_all_objects(ad->maps_view);
	if (ret != MAPS_ERROR_NONE) {
		LS_LOGE("map_view_remove_all_objects fail, error = %d", ret);
		return;
	}
	ret = map_object_create_marker(maps_coord, IMG_DIR"/marker_icon.png", MAP_MARKER_PIN, &marker);
	if (ret != MAPS_ERROR_NONE) {
		LS_LOGE("map_object_create_marker fail, error = %d", ret);
		return;
	}

	ret = map_view_add_object(ad->maps_view, marker);
	if (ret != MAPS_ERROR_NONE) {
		LS_LOGE("map_view_add_object fail, error = %d", ret);
		return;
	}
...
static void mapLongPressed(myplace_app_data *ad)
{
	...
	ret = map_object_create_marker(coordinates, IMG_DIR"/marker_icon.png", MAP_MARKER_PIN, &marker);
	if (ret != MAPS_ERROR_NONE) {
		LS_LOGE("map_object_create_marker fail, error = %d", ret);
		return;
	}
...
static void map_view_event_cb(maps_error_e result, const map_event_type_e type, map_event_data_h event_data, void *user_data)
{
	myplace_app_data *ad = (myplace_app_data *)user_data;
	map_gesture_e gesture_type = MAP_GESTURE_NONE;
	int ret = MAPS_ERROR_NONE;

	LS_LOGE("map_view_event_cb enter");

	map_event_data_get_gesture_type(event_data, &gesture_type);

	switch (type) {
	case MAP_EVENT_GESTURE:
		LS_LOGE("MAP_EVENT_GESTURE");
		switch (gesture_type) {
		case MAP_GESTURE_LONG_PRESS: {
			LS_LOGE("MAP_GESTURE_LONG_PRESS");
			maps_coordinates_h event_coord = NULL;
			int x = 0, y = 0;
			double latitude = 0.0, longitude = 0.0;
			ret = map_event_data_get_position(event_data, &x, &y);
			if (ret != MAPS_ERROR_NONE) {
				LS_LOGE("map_event_data_get_position is fail[%d]", ret);
				break;
			}
			ret = map_view_screen_to_geolocation(ad->maps_view, x, y, &event_coord);
			if (ret != MAPS_ERROR_NONE) {
				LS_LOGE("map_view_screen_to_geolocation is fail[%d]", ret);
				break;
			}
...
static bool searchOnMapsService_cb(maps_error_e result, int request_id, int index, int total, maps_coordinates_h coordinates, void *user_data)
{
	...
	ret = map_view_set_zoom_level(ad->maps_view, 12);
	if (ret != MAPS_ERROR_NONE) {
		LS_LOGE("map_view_set_zoom_level fail, error = %d", ret);
		return false;
	}
	ret = map_view_set_center(ad->maps_view, coordinates);
	if (ret != MAPS_ERROR_NONE) {
		LS_LOGE("map_view_set_center fail, error = %d", ret);
		return false;
	}
...
static void mapview_cancel_cb(void *data, Evas_Object * obj, void *event_info)
{
	...
	if (ad->maps_view != NULL) {
		...
		/* Unset all callbacks */
		ret = map_view_unset_event_cb(ad->maps_view, MAP_EVENT_GESTURE);
		if (ret != MAPS_ERROR_NONE)
			LS_LOGE("map_view_unset_event_cb fail, error = %d", ret);
		ret = map_view_unset_event_cb(ad->maps_view, MAP_EVENT_OBJECT);
		if (ret != MAPS_ERROR_NONE)
			LS_LOGE("map_view_unset_event_cb fail, error = %d", ret);

		/*
		 *  WARNING! It is not mandatory to remove objects manually,
		 *  they will be removed automatically by Map View
		 */
		...
		ret = map_view_destroy(ad->maps_view);
		show_maps_error(ret);
		ad->maps_view = NULL;
	}
...
static Evas_Image *create_map_layout(Evas_Object *parent, myplace_app_data *ad)
{
	...
	ret = map_view_create(ad->maps_service, panel, &(ad->maps_view));
	if (ret != MAPS_ERROR_NONE) {
		LS_LOGE("map_view_create fail, error = %d", ret);
		return NULL;
	}
	...
	if (ad->mapview_place->address == NULL) {
		...
		map_view_set_zoom_level(ad->maps_view, 2);
	} else {
	...
	ret = map_view_set_event_cb(ad->maps_view, MAP_EVENT_GESTURE, map_view_event_cb, ad);
	if (ret != MAPS_ERROR_NONE) {
		LS_LOGE("map_view_set_event_cb fail, error = %d", ret);
		return NULL;
	}
	ret = map_view_set_event_cb(ad->maps_view, MAP_EVENT_OBJECT, map_view_event_cb, ad);
	if (ret != MAPS_ERROR_NONE) {
		LS_LOGE("map_view_set_event_cb fail, error = %d", ret);
		return NULL;
	}
