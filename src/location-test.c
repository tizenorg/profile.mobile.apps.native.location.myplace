...
static char *TYPE_METHOD[] ={"HYBRID", "GPS", "WPS", "-MOCK", "-FUSED", "-PASSIVE"};
static char *TYPE_INTERVAL[] ={"1", "2", "3", "4", "5", "10", "20", "50", "100"};
static char *TYPE_PERIOD[] ={"1", "2", "3", "5", "7", "10", "20", "50", "120"};
static int INT_INTERVAL[]	= { 1, 2, 3, 4, 5, 10, 20, 50, 100};
...
static bool start_status = false, setting_status = false, mock_status = false;
...
//--------------------------------------------------------------------------
static void _remove_callbacks()
{
	if (cb_pos) {	location_manager_unset_position_updated_cb(manager); cb_pos = false;}
	if (cb_vel) {	location_manager_unset_velocity_updated_cb(manager); cb_vel = false;}
	if (cb_loc) {	location_manager_unset_location_changed_cb(manager); cb_loc = false;}
	if (cb_dist) {	location_manager_unset_distance_based_location_changed_cb(manager); cb_dist = false;}
	if (cb_sat) {	gps_status_unset_satellite_updated_cb(manager); cb_sat = false;}
}
...
static void _popup_start_stop_cb(void *data, Evas_Object *obj, void *event_info)
{
	...
	if (idx == 0) {
		...
		view_log(vlog, "<font_size=17>Location_create[%s].. </font_size>", TYPE_METHOD[METHOD]);
	} else if (idx == 1) {
...
static void _popup_satellite_cb(void *data, Evas_Object *obj, void *event_info)
{
	...
	if (idx == 0) {
		...
		if (setting_status)
			location_manager_enable_method(LOCATIONS_METHOD_GPS, false);
		else
			location_manager_enable_method(LOCATIONS_METHOD_GPS, true);
	} else if (idx == 4) {
		if (setting_status)
			location_manager_enable_method(LOCATIONS_METHOD_WPS, false);
		else
			location_manager_enable_method(LOCATIONS_METHOD_WPS, true);
	}
	show_result(ret, ad);
}
static void _popup_satellite(void *data, Evas_Object *obj, void *event_info)
{
	...
	char *TYPE_API[] ={"get_satellite", "foreach", "-------------", "GPS setting", "WPS setting"};
	for(i = 0; i < sizeof(TYPE_API) / sizeof(TYPE_API[0]); i++) {
		if (i < 3) {
			elm_ctxpopup_item_append(type_pop, TYPE_API[i], NULL, _popup_satellite_cb, (void *)i);
		} else if (i == 3) {
			location_manager_is_enabled_method(LOCATIONS_METHOD_GPS, &setting_status);
			if (setting_status)
				elm_ctxpopup_item_append(type_pop, "GPS: On ", NULL, _popup_satellite_cb, (void *)i);
			else
				elm_ctxpopup_item_append(type_pop, "GPS: Off", NULL, _popup_satellite_cb, (void *)i);

		} else if (i == 4) {
			location_manager_is_enabled_method(LOCATIONS_METHOD_WPS, &setting_status);
			if (setting_status)
				elm_ctxpopup_item_append(type_pop, "WPS: On ", NULL, _popup_satellite_cb, (void *)i);
			else
				elm_ctxpopup_item_append(type_pop, "WPS: Off", NULL, _popup_satellite_cb, (void *)i);
		}
	}
...
//--------------------------------------------------------------------------
static void _popup_pub_cb(void *data, Evas_Object *obj, void *event_info)
{
	int idx = (int) data;
	evas_object_del(type_pop);
	...
	if (idx == 0) {
		if (mock_status) {
			...
			view_log(vlog, "<font_size=17>enable_mock_locatioin [FALSE] ... </font_size>");
			mock_status = false;
		} else {
			ret = location_manager_enable_mock_location(true);
			view_log(vlog, "<font_size=17>enable_mock_locatioin [TRUE] ... </font_size>");
			mock_status = true;
		}
	} else if (idx == 1) {
		ret = location_manager_set_mock_location(manager, 10.0, 21.1, 32.2, 40.0, 51.1, 6.0);
		view_log(vlog, "<font_size=17>set_mock_location ... </font_size>");
	} else if (idx == 2) {
		ret = location_manager_clear_mock_location(manager);
		view_log(vlog, "<font_size=17>clear_mock_location ... </font_size>");
	} else if (idx == 3) {
		for (i = 0; i < PERIOD; i++) {
			ret = location_manager_create(METHOD, &manager);
			view_log(vlog, "<font_size=17>location_create_%d [%s] .. <br></font_size>", i + 1, TYPE_METHOD[METHOD]);
			ret = location_manager_set_service_state_changed_cb(manager, __state_changed_cb, NULL);
			ret = gps_status_set_satellite_updated_cb(manager, __satellite_updated_cb, INTERVAL, NULL);
			ret = location_manager_set_location_changed_cb(manager, __location_auto_cb, INTERVAL, NULL);
			ret = location_manager_start(manager);
			wait_for_response();
...
#ifdef TIZEN_FEATURE_FUSED
	} else if (idx == 4) {
		ret = location_manager_enable_fused_interval(manager, "false");
		view_log(vlog, "<font_size=17>fused_interval[Adaptive interval: False] ... </font_size>");
	} else if (idx == 5) {
		ret = location_manager_fused_accuracy_mode(manager, LOCATIONS_FUSED_HIGH);
		view_log(vlog, "<font_size=17>fused_mode [High accuracy] ... </font_size>");
	} else if (idx == 6) {
		ret = location_manager_fused_accuracy_mode(manager, LOCATIONS_FUSED_BALANCED);
		view_log(vlog, "<font_size=17>fused_mode [Balanced power] ... </font_size>");
	} else if (idx == 7) {
		ret = location_manager_fused_accuracy_mode(manager, LOCATIONS_FUSED_NO_POWER);
		view_log(vlog, "<font_size=17>fused_mode [Balanced power] ... </font_size>");
#endif
	}
	show_result(ret, ad);
}
static void _popup_pub(void *data, Evas_Object *obj, void *event_info)
{
	if (type_pop) {
		evas_object_del(type_pop); type_pop = NULL;
	}
	...
	char *TYPE_API[] ={"enable_mock", "set_mock", "clear_mock", "Auto test"
#ifdef TIZEN_FEATURE_FUSED
	, "Fintval_false", "fused-high", "fused-balanced", "fused-nopower"
#endif
						};

	for(i = 0; i < sizeof(TYPE_API) / sizeof(TYPE_API[0]); i++) {
		if (i > 0) {
			elm_ctxpopup_item_append(type_pop, TYPE_API[i], NULL, _popup_pub_cb, (void *)i);
		} else {
			if (mock_status)
				elm_ctxpopup_item_append(type_pop, "mock: true ", NULL, _popup_pub_cb, (void *)i);
			else
				elm_ctxpopup_item_append(type_pop, "mock: false", NULL, _popup_pub_cb, (void *)i);
		}
	}
...
static const char* get_wifi(int state)
{
	switch (state) {
	case VCONFKEY_NETWORK_WIFI_OFF: return "off";
	case VCONFKEY_NETWORK_WIFI_NOT_CONNECTED: return "not_co";
	case VCONFKEY_NETWORK_WIFI_CONNECTED: return "con";
	default: return "inva";
	}
}
static const char* get_dnet(int state)
{
	switch (state) {
	case VCONFKEY_DNET_OFF: return "off";
	case VCONFKEY_DNET_NORMAL_CONNECTED: return "nor_co";
	case VCONFKEY_DNET_SECURE_CONNECTED: return "scr_co";
	case VCONFKEY_DNET_TRANSFER: return "trans";
	default: return "inva";
	}
}
...
static void _show_position_status()
{
	...
	vconf_get_int(VCONFKEY_NETWORK_WIFI_STATE, &wifi);
	vconf_get_int(VCONFKEY_DNET_STATE, &dnet);
	view_log(vlog, "<color=#9F000F><font_size=18> ========= GPS [%s]   WPS [%s: %s, %s]   Pos [%s] </font_size></color>", get_state(gps), get_state(wps), get_wifi(wifi), get_dnet(dnet), get_state(pos));
	show_result(ret, ad);
}
...
static void _vconf_wifi_changed_cb(keynode_t *key, void *data){ _show_position_status();}
static void _vconf_dnet_changed_cb(keynode_t *key, void *data){ _show_position_status();}
...
static void __setting_location_back_cb(void *data, Evas_Object *obj, void *event_info)
{
	...
	vconf_ignore_key_changed(VCONFKEY_NETWORK_WIFI_STATE, _vconf_wifi_changed_cb);
	vconf_ignore_key_changed(VCONFKEY_DNET_STATE, _vconf_dnet_changed_cb);
	if (start_status) {
		ret = location_manager_stop(manager);
		start_status = false;
	}
...
void _setting_location_test_view(void *data, Evas_Object *obj, void *event_info)
{
	...
	Evas_Object *box, *method, *interval, *startstop, *callback, *get, *batch, *period, *satellite, *etc, *pub;
...
///////////////////////////////////////////
	...
	elm_object_text_set(etc, ("<font_size=20>Log:state</font_size>"));
	...
	elm_object_text_set(pub, ("<font_size=20>3.0 api</font_size>"));
	evas_object_smart_callback_add(pub, "clicked", _popup_pub, ad);
	evas_object_resize(pub, 120, 50); evas_object_move(pub, 580, 230); evas_object_show(pub);
...
//	evas_object_resize(, 120, 50); evas_object_move(, 300, 230); evas_object_show();
	...
	vconf_notify_key_changed(VCONFKEY_NETWORK_WIFI_STATE, _vconf_wifi_changed_cb, NULL);
	vconf_notify_key_changed(VCONFKEY_DNET_STATE, _vconf_dnet_changed_cb, NULL);
...
{
	view_log(vlog, "<color=#3090C7><font_size=16>Pos [lat:%.1lf, log:%.1lf, alt:%.1lf, %d]<br></font_size></color>", latitude, longitude, altitude, timestamp);
}

void __velocity_updated_cb(double speed, double direction, double climb, time_t timestamp, void *user_data)
{
	view_log(vlog, "<color=#9090C7><font_size=16>VEL [spd:%.1lf, dir:%.1lf, climb:%.1lf, %d]-------<br></font_size></color>", speed, direction, climb, timestamp);
}
...
{
	view_log(vlog, "<color=#303077><font_size=16>LOCATION!! [Pos:%.1lf, %.1lf, %.1lf, Vel:%.1lf, %.1lf, %.1lf, %d]<br></font_size></color>", latitude, longitude, altitude, speed, direction, climb, timestamp);
}
...
{
	view_log(vlog, "<color=#406077><font_size=16>=== Changed!! [Pos:%.1lf, %.1lf, %.1lf, Vel:%.1lf, %.1lf, H_Acc%.1lf, %d]<br></font_size></color>", latitude, longitude, altitude, speed, direction, hor_accuracy, timestamp);
}

void __location_auto_cb(double latitude, double longitude, double altitude, double speed, double direction, double hor_accuracy, time_t timestamp, void *user_data)
{
	view_log(vlog, "<color=#406077><font_size=16>=== Changed!! [Pos:%.1lf, %.1lf, %.1lf, Vel:%.1lf, %.1lf, H_Acc%.1lf, %d]<br></font_size></color>", latitude, longitude, altitude, speed, direction, hor_accuracy, timestamp);
	if (g_mainloop) {
...
{
	view_log(vlog, "<color=#6060C7><font_size=16>AA_Pos [lat:%.1lf, lon:%.1lf, alt:%.1lf, %d]<br></font_size></color>", latitude, longitude, altitude, timestamp);
}
...
{
	view_log(vlog, "<color=#A04000><font_size=16>Satellite_updated!! [active: %d, inview: %d, %d]<br></font_size></color>", num_of_active, num_of_inview, timestamp);
...
{
	view_log(vlog, "<color=#F5B041><font_size=16>get_satellite!! [azimuth:%d, elevation:%d, prn:%d, snr:%d, is_in_use:%d]<br></font_size></color>", azimuth, elevation, prn, snr, is_in_use);
	return true;
}
...
{
	view_log(vlog, "<color=#9F000F><font_size=16>Batch_updated !! [size: %d]<br></font_size></color>", num_of_location);
	location_manager_foreach_location_batch(manager, __get_batch_cb, NULL);
}
...
{
	view_log(vlog, "<color=#406077><font_size=16>  get batch [%ld, %.1lf, %.1lf, %.1lf, spd: %.1lf, %.1lf, acc: %.1lf, %.1lf]<br></font_size></color>", timestamp, latitude, longitude, altitude, speed, direction, horizontal, vertical);
	return true;
}
...
void show_result(int result, void *data)
{
	...
	else view_log(vlog, "<color=#9F000F><font_size=15>[%s=%d]<br></font_size></color>", result, "INVALID");
}
